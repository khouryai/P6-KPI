...
export function buildWorkbook(model: Model, settings: Settings, current: P6Activity[], baseline: P6Activity[], testProgress: TestProgress[], snapshots: Snapshot[]): XLSX.WorkBook {
  ...
  XLSX.utils.book_append_sheet(wb, wsFrom([
    ['Row', 'P6_Activity_ID', 'Location', 'Seq_Code', 'Match_Key', 'Rate_Status', 'Status', 'Basis', 'LOE_Flag', 'Complexity', 'Std_Hours', 'Override_Hours', 'Budget_Hours', 'Baseline_Start', 'Baseline_Finish', 'Baseline_Source', 'Current_Start', 'Current_Finish', 'Pct_Complete', 'Pct_Source', 'Earned_Hours', 'Remaining_Hours', 'Match_Tier', 'Activity_Name', 'Activity_Type', 'Discipline', 'Earn_Start', 'Earn_End', 'Earn_Window_Source'],
    ...model.rows.map((r, i) => [i + 2, r.activity.rawActivityId, r.location, r.seqCode, r.matchKey, r.rateStatus, r.status, r.basis ?? '', r.loeFlag ? 'LOE?' : '', r.complexity ?? '', r.stdHours ?? '', r.overrideHours ?? '', r.budgetHours, d(r.baselineStart), d(r.baselineFinish), r.baselineSource, d(r.currentStart), d(r.currentFinish), r.pctComplete, r.pctSource, r.earnedHours, r.remainingHours, r.matchTier ?? '', r.activity.activityName, r.activityType, r.discipline, d(r.earnStart), d(r.earnEnd), r.earnWindowSource]),
  ]), 'Budget_Master');
  XLSX.utils.book_append_sheet(wb, wsFrom([

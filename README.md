...
| A fortnightly review | **Two-Week Log**: what the baseline planned for the period, what was actually achieved, and every activity behind it — completed, started, continued, missed or never started. Achievement is reported for each phase as well as for the whole job. Every missed activity gets a **why**, picked from a list you extend from the dropdown itself and prune under **Reasons** (anything nobody has used yet can be deleted, the app's own suggestions included), and the reasons are totalled on the screen. Test counts can be keyed straight from a row. Steps period by period, and copies as text for a report. |
